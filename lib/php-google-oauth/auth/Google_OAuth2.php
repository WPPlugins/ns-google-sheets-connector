  ...
  public function authenticate($service, $code = null) {
    ...
    if ($code) {
      ...
      if ($request->getResponseHttpCode() == 200) {
		...
		ns_google_sheets_connector_pro::ns_debug_log("Error fetching OAuth2 access token, message: '$response' :: {$request->getResponseHttpCode()}");
    ...
    if ($this->isAccessTokenExpired()) {
      if ($this->assertionCredentials) {
        ...
        if (! array_key_exists('refresh_token', $this->token)) {
			ns_google_sheets_connector_pro::ns_debug_log("The OAuth 2.0 access token has expired, "
