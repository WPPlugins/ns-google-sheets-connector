...
class googlesheet {
	...
	public static function updateToken($tokenData){
		...
		try{
			...
			update_option('ns_google_sheets_connector_pro_token', $tokenJson);
			
		} catch (Exception $e) {
			ns_google_sheets_connector_pro::ns_debug_log("Token write fail!");
		}
	}
	
	public function auth(){
		// $tokenData = json_decode(file_get_contents(plugin_dir_path(__FILE__) . ".tokendata"), true);
		$tokenData = json_decode( get_option('ns_google_sheets_connector_pro_token', null), true);
