...
class ns_google_sheets_connector {
	...
	function __construct(){		
		...
		//Deactivates current API code
		if(isset($_GET['remove_code'])) {
			update_option( $this->ns_plugin_ref.'_set', '');
		}
		...
		switch($type_value){
			...
			default:
				...
				if ( empty($saved_value) && $args['field_name'] == "{$this->ns_plugin_ref}_code" && get_option($this->ns_plugin_ref.'_set')) {?>
					<p style="margin-top:5px;"><i><?php _e( 'Currently Activated', $this->text_domain);?></i></p>
					<a class="button" style="
						margin-top:5px;
						background-color:#d54e21;
						color:#fff;
						-webkit-box-shadow: 0 1px 0 #9c3817;
						box-shadow: 0 1px 0 #9c3817;
						border-color:#9c3817;"
						href="<?php echo (isset($_SERVER['HTTPS']) ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";?>&remove_code=yes"><?php _e( 'Deactivate', $this->text_domain );?></a>
				<?php }else if ( empty($saved_value) && !$val_set ) {
					echo '<input type="' . $type_value . '" name="' . $args['field_name'] . '" value="" /><br/>';
					echo '<i>Click <a href="https://accounts.google.com/o/oauth2/auth?access_type=offline&approval_prompt=force&client_id=1058344555307-fcus00minenokgq9vm48toli90q22783.apps.googleusercontent.com&redirect_uri=urn%3Aietf%3Awg%3Aoauth%3A2.0%3Aoob&response_type=code&scope=https%3A%2F%2Fspreadsheets.google.com%2Ffeeds%2F" target="_blank">here</a> to retrieve your code from Google Drive to allow us to access your spreadsheets.</i>';
